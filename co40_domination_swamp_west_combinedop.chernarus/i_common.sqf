...
#endif
GVAR(AmmoBoxHandling) = (GVAR(AmmoBoxHandling) == 1);
...
//default flag GUER
#ifdef __OWN_SIDE_WEST__
FLAG_BASE setflagtexture (if !(__A2Ver) then {"\ca\Ca_E\data\flag_us_co.paa"} else {"\ca\data\flag_usa_co.paa"});
#endif
#ifdef __OWN_SIDE_EAST__
switch (true) do {
	case (__OAVer): {FLAG_BASE setflagtexture "\ca\Ca_E\data\flag_tka_co.paa"};
	case (__COVer): {FLAG_BASE setflagtexture "\ca\data\flag_rus_co.paa"};
};
#endif
...
if (GVAR(with_dlc)) then {
	if (GVAR(own_side) == "WEST") then {
		if (!(__ACEVer) && __OAVer) then {
			GVAR(sm_bonus_vehicle_array) = GVAR(sm_bonus_vehicle_array) + ["BAF_FV510_D","BAF_Jackal2_GMG_D","BAF_Jackal2_L2A1_D","BAF_Apache_AH1_D","AW159_Lynx_BAF","Ka60_GL_PMC"];
		};
	};
};
#endif
...
if (GVAR(with_dlc)) then {
	if (GVAR(own_side) == "WEST") then {
		if (!(__ACEVer) && __OAVer) then {
			GVAR(mt_bonus_vehicle_array) = GVAR(mt_bonus_vehicle_array) + ["ArmoredSUV_PMC"];
		};
	};
};
#endif
...
if (GVAR(AmmoBoxHandling)) then {GVAR(last_ammo_drop) = -3423};
