...
GVAR(x_sm_pos) = [[7830.44,13149.4,0]]; //  cruise missile
...
if (X_Client) then {
	GVAR(current_mission_text) = "One of our planes has lost a cruise missile in the northern territories, bad luck for us, it didn't explode. Find it and destroy it before the enemy gets it's hands on it. Be aware they are also heading for the crash site!";
	GVAR(current_mission_resolved_text) = "Good job. The cruise missile is down.";
};
		...
		if (_time_over > 0) then {
			...
			if (!_enemy_created) then {
				...
				for "_i" from 1 to _ran do {
					...
					extra_mission_remover_array = [extra_mission_remover_array, _units] call FUNC(arrayPushStack);
					sleep 1.012;
				};
				_unit_array = nil;
			};
