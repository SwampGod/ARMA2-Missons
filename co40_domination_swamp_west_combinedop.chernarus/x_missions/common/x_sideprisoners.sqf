	...
	while {!_hostages_reached_dest && !_all_dead} do {
		...
		if ((_units call FUNC(GetAliveUnits)) == 0) then {
			...
			if (!_rescued) then {
				...
				if (count _nobjs > 0) then {
					{
						if (isPlayer _x && (toUpper (str(_x)) in ["RESCUE","RESCUE2"]) && alive _x) exitWith {
							...
							[QGVAR(joing), [_units, group _retter]] call FUNC(NetCallEvent);
						};
						sleep 0.01;
					} forEach _nobjs;
				};
				...
				_do_loop = true;
				{
					if (!isNull _x && alive _x) then {
						if (!__TTVer) then {
							if ((vehicle _x) distance FLAG_BASE < 20) then {
								_hostages_reached_dest = true;
								_do_loop = false;
							};
						} else {
							if ((vehicle _x) distance WFLAG_BASE < 20) then {
								_hostages_reached_dest = true;
								_do_loop = false;
								_winner = 2;
							} else {
								if ((vehicle _x) distance EFLAG_BASE < 20) then {
									_hostages_reached_dest = true;
									_do_loop = false;
									_winner = 1;
								};
		...
		if (__RankedVer) then {
			if (_hostages_reached_dest) then {
				if (!__TTVer) then {
					[QGVAR(sm_p_pos), position FLAG_BASE] call FUNC(NetCallEvent);
				} else {
					switch (_winner) do {
						case 1: {[QGVAR(sm_p_pos), position EFLAG_BASE] call FUNC(NetCallEvent)};
						case 2: {[QGVAR(sm_p_pos), position WFLAG_BASE] call FUNC(NetCallEvent)};
					};
				};
			};
		};
	...
	while {!_hostages_reached_dest && !_all_dead} do {
		...
		if ((_units call FUNC(GetAliveUnits)) == 0) then {
			...
			if (!_rescued) then {
				...
				{
					if (!isNull _x && alive _x && (vehicle _x) distance FLAG_BASE < 20) exitWith {_hostages_reached_dest = true};
				} forEach _units;
			};
		};
		if (__RankedVer) then {
			if (_hostages_reached_dest) then {[QGVAR(sm_p_pos), position FLAG_BASE] call FUNC(NetCallEvent)};
		};
		sleep 5.123;
	};
};
