...
_fixor = {
	...
	while {true} do {
		...
		#ifndef __TT__
		if ((X_JIPH getVariable QGVAR(campscaptured)) == GVAR(sum_camps) && ("Car" countType _list <= GVAR(car_count_for_target_clear)) && ("Tank" countType _list <= GVAR(tank_count_for_target_clear)) && ("Man" countType _list <= GVAR(man_count_for_target_clear))) exitWith {};
		#else
		if (((X_JIPH getVariable QGVAR(campscaptured_e)) == GVAR(sum_camps) || (X_JIPH getVariable QGVAR(campscaptured_w)) == GVAR(sum_camps)) && ("Car" countType _list <= GVAR(car_count_for_target_clear)) && ("Tank" countType _list <= GVAR(tank_count_for_target_clear)) && ("Man" countType _list <= GVAR(man_count_for_target_clear))) exitWith {};
		#endif
...
switch (_sec_kind) do {
	case 1: {
		...
		if (_sum < count _iar) then {
			...
			[QGVAR(s_b_client)] call FUNC(NetCallEvent);
		} else {
	...
	case 3: {
		...
		_vehicle setPos _poss;
		_vehicle lock true;
		_vehicle addEventHandler ["killed", {_this set [count _this, "ammo_down"]; _this call FUNC(MTSMTargetKilled); [_this select 0] call FUNC(handleDeadVec)}];
		[_vehicle, __XJIPGetVar(GVAR(current_target_index))] spawn _fixor;
		sleep 1.0112;
		__specops;
		#ifdef __TT__
	...
	case 4: {
		...
		if (__ACEVer) then {
			...
			_vehicle addEventHandler ["killed", {_this set [count _this, "apc_down"]; _this call FUNC(MTSMTargetKilled); [_this select 0] call FUNC(handleDeadVec)}];
		};
		[_vehicle, __XJIPGetVar(GVAR(current_target_index))] spawn _fixor;
		sleep 1.0112;
		__specops;
