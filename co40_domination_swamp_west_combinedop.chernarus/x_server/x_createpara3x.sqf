...
#ifdef __TT__
				_one_unit addEventHandler ["killed", {[[15, 3, 2, 1],_this select 1, _this select 0] call FUNC(AddKills)}];
#endif
...
for "_i" from 1 to _number_vehicles do {
	...
	if !(_heli_type in GVAR(heli_wreck_lift_types)) then {__addDead(_vehicle)};
#ifdef __TT__
