...
if (GVAR(with_ai)) then {
	if (__RankedVer) then {
		FUNC(AddKillsAI) = {
			private ["_points","_killer"];
			PARAMS_2(_points,_killer);
			if (!isPlayer _killer && side (group _killer) != GVAR(side_enemy)) then {[QGVAR(ai_kill), [_killer,_points]] call FUNC(NetCallEvent)}
		};
	};
};
...
FUNC(makevgroup) = {
	private ["_numbervehicles", "_pos", "_vehiclename", "_grp", "_direction", "_crews", "_grpskill", "_n", "_dir", "_vehicle", "_npos"];
	...
	for "_n" from 0 to _numbervehicles - 1 do {
		...
		_crews = [_crews, _vec_array select 1] call FUNC(arrayPushStack);
		...
		switch (true) do {
			case (_vehicle isKindOf "Tank"): {
				if !(_vehiclename in GVAR(heli_wreck_lift_types)) then {
					#ifdef __ACE__
					__addRemoveVehi(_vehicle)
					#endif
					_vehicle addEventHandler ["killed",{[_this select 0] call FUNC(handleDeadVec)}];
					__addDead(_vehicle)
				};
...
FUNC(makemgroup) = {
	 ...
	 _do_points = (count _this > 3);_ret = [];
	{
		...
		#ifdef __TT__
		if (_do_points) then {_one_unit addEventHandler ["killed", {[[15, 3, 2, 1],_this select 1, _this select 0] call FUNC(AddKills)}]};
		#endif
		if (GVAR(with_ai)) then {if (__RankedVer) then {_one_unit addEventHandler ["killed", {[1,_this select 1] call FUNC(AddKillsAI)}]}};
	...
	for "_nr" from 0 to 2 do {
		...
		if (_nrg > 0) then {
			...
			for "_i" from 1 to _nrg do {
				...
				GVAR(extra_mission_vehicle_remover_array) = [GVAR(extra_mission_vehicle_remover_array), _reta select 0] call FUNC(arrayPushStack);
...
FUNC(MakePatrolWPX) = {
	private ["_grp", "_start_pos", "_wp_array", "_i", "_wp_pos", "_counter", "_wp", "_wppos", "_pos", "_cur_pos","_no_pos_found", "_wpstatements"];
...
FUNC(DelVecAndCrew) = {{deleteVehicle _x} forEach ([[_this], crew _this] call FUNC(arrayPushStack))};
...
FUNC(PlacedObjKilled) = {
	...
	if (!isNil "_val") then {
		...
		[QGVAR(p_o_an), _val] call FUNC(NetCallEvent);
	};
};
...
FUNC(TKKickCheck) = {
	...
	if (!isNil "_p") then {
		...
		if (_numtk >= GVAR(maxnum_tks_forkick)) exitWith {
			...
			[QGVAR(tk_an), [_pna, _numtk]] call FUNC(NetCallEvent);
			[QGVAR(em), _tk] call FUNC(NetCallEvent);
		};
#ifdef __ACE__
		[QGVAR(haha), _tk] call FUNC(NetCallEvent);
#endif
	};
};
...
FUNC(KickPlayerBS) = {
	...
	[QGVAR(ps_an), [_pl_name, _reason]] call FUNC(NetCallEvent);
};
...
FUNC(GetPlayerPoints) = {
	...
	_uid = _this;
	_pa = GV2(GVAR(player_store),_uid);
	__TRACE_1("fnc_GetPlayerPoints","_uid")
	if (!isNil "_pa") then {[QGVAR(p_ar), _pa] call FUNC(NetCallEvent)};
};

FUNC(GetAdminArray) = {
	private "_h";
	_h = GV2(GVAR(player_store),_this);
	if (isNil "_h") then {_h = []};
	[QGVAR(s_p_inf), _h] call FUNC(NetCallEvent);
};
...
FUNC(RemABox) = {
	...
	{
		if ((_x select 0) distance _this < 5) exitWith {_x set [0,[]]};
	} forEach _boxes;
};
...
FUNC(SideMissionResolved) = {
	...
	if (GVAR(side_mission_winner) in [-1,-2,-300,-400,-500,-600,-700,-878,-879]) then {
		[QGVAR(sm_res_client), [GVAR(side_mission_winner), -1]] call FUNC(NetCallEvent);
#ifndef __TT__
...
if (!GVAR(AmmoBoxHandling)) then {
	...
	FUNC(CreateDroppedBox) = {
		...
		[_mname, _the_box_pos,"ICON","ColorBlue",[0.5,0.5],"Ammo",0,"mil_marker"] call FUNC(CreateMarkerGlobal);
	};
#else
	FUNC(CreateDroppedBox) = {
		...
		[QGVAR(ammo_boxes),_boxes] call FUNC(NetSetJIP);
		[_mname, _the_box_pos,"ICON","ColorBlue",[0.5,0.5],"Ammo",0,"mil_marker"] call FUNC(CreateMarkerGlobal);
		[QGVAR(r_mark), [_mname, _boxside]] call FUNC(NetCallEvent);
	};
#endif
} else {
	#ifndef __TT__
	...
	FUNC(CreateDroppedBox) = {
		...
		PARAMS_2(_vec,_the_box_pos);
		["ammo_boxes",__XJIPGetVar(ammo_boxes) + 1] call FUNC(NetSetJIP);
		_cbox = [_vec, _the_box_pos];
		GVAR(check_boxes) set [count GVAR(check_boxes), _cbox];
		_boxes = __XJIPGetVar(GVAR(ammo_boxes));
	...
	FUNC(CreateDroppedBox) = {
		...
		PARAMS_3(_vec,_the_box_pos,_boxside);
		["ammo_boxes",__XJIPGetVar(ammo_boxes) + 1] call FUNC(NetSetJIP);
...
FUNC(GetSMTargetMessage) = {
	switch (_this) do {
		...
		#endif
		#ifdef __OA__
		case "radar_down": {"Your team destroyed the fortress."};
		#endif
...
FUNC(AirMarkerMove) = {
	...
	if (!isNull _vec && alive _vec && canMove _vec && !GVAR(banti_airdown)) then {
		...
		[_markern, [0,0,0],"ICON","ColorRed",[0.5,0.5],"Unknown air contact",0,"Air"] call FUNC(CreateMarkerGlobal);
...
FUNC(IsleDefMarkerMove) = {
	...
	if (!isNull _grp && (_grp call FUNC(GetAliveUnitsGrp)) > 0) then {
		...
		[_markern, [0,0,0],"ICON","ColorRed",[0.5,0.5],"Enemy patrol",0,"n_mech_inf"] call FUNC(CreateMarkerGlobal);
...
// by Rommel
FUNC(searchNearby) = {
	private ["_group", "_leader", "_behaviour", "_array", "_building", "_indices", "_count", "_units", "_timeout", "_unit"];
...
FUNC(TKR) = {
	...
	[QGVAR(unit_tk2), [_namep,_namek]] call FUNC(NetCallEvent);
};
