...
if (GVAR(MissionType) == 2 || GVAR(sideMissionBonus) == 1) exitWith {
	[QGVAR(sm_res_client), [GVAR(side_mission_winner),0]] call FUNC(NetCallEvent);
#ifndef __TT__
...
[QGVAR(sm_res_client), [GVAR(side_mission_winner),_bonus_number]] call FUNC(NetCallEvent);
#ifndef __TT__
GVAR(kb_logic1) kbTell [GVAR(kb_logic2),GVAR(kb_topic_side),"MissionAccomplished",true];
#else
if (GVAR(side_mission_winner) == 1) then {
