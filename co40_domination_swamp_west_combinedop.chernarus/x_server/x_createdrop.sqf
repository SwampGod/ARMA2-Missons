...
[QGVAR(dropansw), [0,_player]] call FUNC(NetCallEvent);
...
[QGVAR(drop_marker), [0,0,0],"ICON","ColorBlue",[0.5,0.5],"Air Drop Aircraft",0,"Dot"] call FUNC(CreateMarkerGlobal);
sleep 0.1;
...
_may_exit = false;
sleep 12 + random 12;
[QGVAR(dropansw), [1,_player]] call FUNC(NetCallEvent);
_starttime = time + 300;
...
if (_may_exit) exitWith {[QGVAR(dropansw), [3,_player]] call FUNC(NetCallEvent)};

[QGVAR(dropansw), [2,_player]] call FUNC(NetCallEvent);
...
[_the_chute,_chopper,_drop_type,_drop_pos,_player] spawn {
	...
	[QGVAR(dropansw), [4,_player]] call FUNC(NetCallEvent);
...
waitUntil {sleep 3.331;QGVAR(drop_marker) setMarkerPos position _chopper;(time > _starttime || !canMove _chopper || _chopper distance _end_pos < 400)};
