...
if (__TTVer) then {
	...
	[QGVAR(w_m_c), [_mname,_sav_pos,_type_name,_pside]] call FUNC(NetCallEvent);
	_d_wreck_marker set [count _d_wreck_marker, [_mname,_sav_pos,_type_name,_pside]];
} else {
	[QGVAR(w_m_c), [_mname,_sav_pos,_type_name]] call FUNC(NetCallEvent);
...
[QGVAR(wreck_marker),_d_wreck_marker] call FUNC(NetSetJIP);
[QGVAR(w_ma),_mname] call FUNC(NetCallEvent);
