...

{
	...
	sleep 0.1;
	_veclist = [_veclist, _vecs] call FUNC(arrayPushStack);
	_unitslist = [_unitslist, _reta select 1] call FUNC(arrayPushStack);
...
_completel = [_completel, _veclist] call FUNC(arrayPushStack);
...
[QGVAR(recaptured), [_recap_index, 1]] call FUNC(NetCallEvent);
