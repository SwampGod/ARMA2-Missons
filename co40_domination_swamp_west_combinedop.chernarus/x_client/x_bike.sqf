...
if (GVAR(with_ranked)) then {
	...
	if (_dosearch) then {
		if (score player < (GVAR(ranked_a) select 6)) then {
			...
			_exitit = true;
			(format["You have to be at least %2 to create a %1",_create_bike,_rank]) call FUNC(GlobalChat);
		};
	};
};
*/
...
if (__pGetVar(GVAR(bike_created)) && _b_mode == 1) exitWith {"Creating a vehicle is only possible once after respawn..." call FUNC(GlobalChat)};
...
if (!isNull GVAR(flag_vec) && alive GVAR(flag_vec) && _b_mode == 0) exitWith {
	(format ["You have allready created a vehicle, find it and use it again!!! Or wait %1 minutes before you can create a new one (might take longer)!",0 max (round((GVAR(vec_end_time) - diag_tickTime)/60))]) call FUNC(GlobalChat);
};

if (GVAR(with_ranked)) then {
	[QGVAR(pas), [player, (GVAR(ranked_a) select 5) * -1]] call FUNC(NetCallEvent);
};
...
if (_vehicle isKindOf "ATV_US_EP1") then {
	_vehicle addAction ["Flip ATV" call FUNC(BlueText), "x_client\x_flipatv.sqf", 0, -1, false, false, "", "!(player in _target) && ((vectorUp _target) select 2) < 0.6"];
};
...
if (_b_mode == 1) then {
	_vehicle spawn {
		private "_vehicle";
		_vehicle = _this;
		[QGVAR(ad), _vehicle] call FUNC(NetCallEvent);
	...
	[QGVAR(ad2), [GVAR(flag_vec),GVAR(vec_end_time)]] call FUNC(NetCallEvent);
