...
if (!(if (!isNil {GV(GVAR(curvec_dialog),ace_canmove)}) then {GVAR(curvec_dialog) call ace_v_alive} else {true})) exitWith {
	"Vehicle destroyed!!!" call FUNC(GlobalChat)
};
#endif
...
if ((vehicle player) in (list GVAR(base_trigger)) && _isthere) exitWith {"You can't deploy a MHQ at base!!!" call FUNC(GlobalChat)};

if (surfaceIsWater (position GVAR(curvec_dialog))) exitWith {"You can't deploy a MHQ here!!!" call FUNC(GlobalChat)};
...
if (_depltime > time) exitWith {"Wait a few seconds before you can deploy/undeploy the MHQ again!" call FUNC(GlobalChat)};

_hxhx = GV(_mhq,GVAR(MHQ_Deployed));
if (isNil "_hxhx") then {_hxhx = false};
if (!_hxhx) then {
	if (count (crew _mhq) > 0) exitWith {"You can't deploy a MHQ when there is still crew on board!" call FUNC(GlobalChat)};
