...
if (GVAR(jumpflag_vec) == "") then {
	_flag addAction ["(Choose Parachute location)" call FUNC(BlueText),"AAHALO\x_paraj.sqf"];
} else {
	_flag addAction [(format ["(Create %1)",[GVAR(jumpflag_vec),0] call FUNC(GetDisplayName)]) call FUNC(BlueText),"x_client\x_bike.sqf",[GVAR(jumpflag_vec),1]];
};
