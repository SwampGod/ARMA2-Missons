...
_vec addEventHandler ["getout", {_this spawn FUNC(checkhelipilotout)}]
...
#define __staticl \
_vec addAction["Load Static" call FUNC(GreyText),"scripts\load_static.sqf",_d_vec,-1,false];\
_vec addAction["Unload Static" call FUNC(RedText),"scripts\unload_static.sqf",_d_vec,-1,false]

#define __addchopm _vec addAction ["Chopper Menu" call FUNC(GreyText),"x_client\x_vecdialog.sqf",[],-1,false]
#define __halo _vec addAction ["HALO Jump" call FUNC(GreyText),"x_client\x_halo.sqf",[],-1,false,true,"","vehicle player != player && ((vehicle player) call d_fnc_GetHeight) > 50"]
...
if (!isNil "_desm") then {
	if (_desm != "") then {
		[_desm, getPosASL _vec,"ICON","ColorBlack",[1,1],"Deserted " + ([typeOf _vec, 0] call FUNC(GetDisplayName)),0,"Dot"] call FUNC(CreateMarkerLocal);
	};
};
...
if (_d_vec < 10) exitWith {
	...
	if (count _car > 0) then {
		...
		if (!isNil {GV(_vec,GVAR(MHQ_Deployed))}) then {
			if (GV(_vec,GVAR(MHQ_Deployed))) then {_mt = _mt + " (Deployed)"};
		};
	...
	if (GVAR(player_side) == west) then {
#endif
	_vec addAction ["MHQ Menu" call FUNC(GreyText),"x_client\x_vecdialog.sqf",_d_vec,-1,false];
#ifdef __TT__
	};
#endif
	...
	_vec setAmmoCargo 0;
#ifdef __A2ONLY__
	_vec addEventhandler ["Killed", {[QGVAR(callMHQF), _this] call FUNC(NetCallEvent)}];
	if (__TTVer) then {
		_vec addEventhandler ["Killed", {[QGVAR(callTTVW), _this] call FUNC(NetCallEvent)}];
	};
#endif
};
...
if (_d_vec < 110) exitWith {
	...
	if (__A2Ver) then {
		_vec addEventhandler ["Killed", {[QGVAR(callMHQF), _this] call FUNC(NetCallEvent)}];
		_vec addEventhandler ["Killed", {[QGVAR(callTTVE), _this] call FUNC(NetCallEvent)}];
	};
};
