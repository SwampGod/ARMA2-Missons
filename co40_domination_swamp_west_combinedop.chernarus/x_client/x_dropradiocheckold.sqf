...
while {true} do {
	...
	__pSetVar [QGVAR(dropaction), player addAction ["Call Drop" call FUNC(GreyText), "x_client\x_calldrop.sqf",[],-1,false]];
	while {isNumber(configFile >> "CfgWeapons" >> secondaryWeapon player >> "ACE_is_radio")} do {
		...
		if (vehicle player != player) then {
			...
			if (player != driver _vec && _vec_id == -8884) then {
				_vec_id = _vec addAction ["Call Drop" call FUNC(GreyText), "x_client\x_calldrop.sqf",[],-1,false];
			} else {
