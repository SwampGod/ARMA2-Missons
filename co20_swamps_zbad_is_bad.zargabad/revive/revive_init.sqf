...
if !(isServer && isDedicated) then
{
	...
	call compile preprocessFile format ["revive\%1_strings_lang.sqf", R3F_REV_CFG_langage];
	...
	[] spawn
	{
		...
		R3F_REV_FNCT_onKilled = compile preprocessFile "revive\onKilled.sqf";
		
		/** Code ramenant le joueur au camp alors qu'il est en attente de réanimation */
		R3F_REV_FNCT_reapparaitre_camp = compile preprocessFile "revive\reapparaitre_camp.sqf";
		...
		if (isClass (configFile >> "CfgVehicles" >> "BAF_Soldier_MTP")) then
		{
			...
			R3F_REV_FNCT_obtenir_infos_sacados = compile preprocessFile "revive\obtenir_infos_sacados.sqf";
			
			/** Assigner le contenu d'un sac à dos */
			R3F_REV_FNCT_assigner_sacados = compile preprocessFile "revive\assigner_sacados.sqf";
		};
		...
		R3F_REV_FNCT_nouvel_inconscient =
		{
			...
			if !(isServer && isDedicated) then 
			{
				if !(isNull _unite) then
				{
					...
					_id_action = _unite addAction [STR_R3F_REV_action_reanimer, "revive\reanimer.sqf", [], 10, false, true, "",
					...
					_id_action = _unite addAction [STR_R3F_REV_action_deplacer_corps, "revive\trainer_corps.sqf", [], 10, false, true, "",
