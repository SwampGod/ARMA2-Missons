...
#else
switch (true) do {
	case (__OAVer): {[_topicside2,"ArtilleryUnAvailable", "Artillery", ["BeAdvisedArtilleryIsUnavailableAtThisTimeOut"]] call FUNC(KBSendMsgAll)};
	case (__COVer): {[_topicside2,"ArtilleryUnAvailable", "Artillery"] call FUNC(KBSendMsgAll)};
};
#endif
sleep 8.54;
#ifndef __TT__
	...
	switch (_ari_type) do {
		...
		case "he": {
			...
			for "_i" from 0 to (_number_shells - 1) do {
				...
				[QGVAR(artyt), [getPosASL _shell, _trails, _netshell]] call FUNC(NetCallEvent);
		...
		case "dpicm": {
			...
			[QGVAR(dpicm), _pos] call FUNC(NetCallEvent);
			sleep 0.5;
			for "_i" from 0 to (_number_shells - 1) do {
				_mpp = _wp_array select _i;
				_shell = createVehicle [_type, _mpp, [], 0, "NONE"];
...
#else
	switch (true) do {
		case (__OAVer): {[_topicside2,"ArtilleryAvailable", "Artillery", ["ArtilleryAvailableTransmitTargetLocationOver"]] call FUNC(KBSendMsgAll)};
		case (__COVer): {[_topicside2,"ArtilleryAvailable", "Artillery"] call FUNC(KBSendMsgAll)};
	};
#endif
};
