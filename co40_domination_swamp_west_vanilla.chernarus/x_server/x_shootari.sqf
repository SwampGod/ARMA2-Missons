...
#ifdef __TT__
	if ((floor random 3) == 0) then {[QGVAR(doarti), _pos_enemy] call FUNC(NetCallEvent)};
#else
	if ((__XJIPGetVar(GVAR(searchintel)) select 5) == 1) then {[QGVAR(doarti), _pos_enemy] call FUNC(NetCallEvent)};
#endif
} else {
	_number_shells = 1;
}; 
...
for "_i" from 0 to (_number_shells - 1) do {
	...
	if (_kind == 1) then {[QGVAR(artyt), [getPosASL _shell, _trails, _netshell]] call FUNC(NetCallEvent)};
	_xo = ceil random 10;
	 sleep 0.923 + (_xo / 10);
};
