...
_objects = _wf nearEntities [["Man", "Tank", "Car"], 12];
_addpwest = 0;
_addpeast = 0;
...
["resolved_targets",_resolved_targets] call FUNC(NetSetJIP);
[QGVAR(mt_winner), GVAR(mt_winner)] call FUNC(NetCallEvent);
...
if (GVAR(current_counter) < GVAR(MainTargets)) then {
	...
	[QGVAR(target_clear), -1] call FUNC(NetCallEvent);
...
GVAR(del_camps_stuff) = [];
{
	...
	_mar = format ["dcamp%1",_x getVariable QGVAR(INDEX)];
	deleteMarker _mar;
