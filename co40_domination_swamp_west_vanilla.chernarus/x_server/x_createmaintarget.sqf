...
for "_xx" from 0 to (count _type_list_guard - 1) do {
	...
	if (_nums > 0) then {
		for "_xxx" from 1 to _nums do {
			...
			_shandle = [_typeidx select 0, [_wp_array select _wp_ran], _trg_center, _typeidx select 1, "guard", GVAR(enemy_side),0,-1.111,1,[_trg_center, _radius]] spawn FUNC(makegroup);
...
for "_xx" from 0 to (count _type_list_guard_static - 1) do {
	...
	if (_nums > 0) then {
		for "_xxx" from 1 to _nums do {
			_wp_ran = (count _wp_array) call FUNC(RandomFloor);
			_shandle = [_typeidx select 0, [_wp_array select _wp_ran], _trg_center, _typeidx select 1, "guardstatic",GVAR(enemy_side),0,-1.111,1,[_trg_center, _radius]] spawn FUNC(makegroup);
			_wp_array set [_wp_ran, -1];
			_wp_array = _wp_array - [-1];
			waitUntil {sleep 0.112;scriptDone _shandle};
			sleep 0.4;
...
for "_xx" from 0 to (count _type_list_guard_static2 - 1) do {
	...
	if (_nrgs > 0) then {
		for "_xxx" from 1 to _nrgs do {
			...
			_shandle = [_typeidx select 0, [_point], _trg_center, _typeidx select 1, "guardstatic2",GVAR(enemy_side),0,-1.111,1,[_trg_center, _radius]] spawn FUNC(makegroup);
			waitUntil {sleep 0.112;scriptDone _shandle};
			sleep 0.1;
		};
	};
...
for "_xx" from 0 to (count _type_list_patrol - 1) do {
	...
	if (_nums > 0) then {
		for "_xxx" from 1 to _nums do {
			_wp_ran = (count _wp_array) call FUNC(RandomFloor);
			_shandle = [_typeidx select 0, [_wp_array select _wp_ran], _trg_center, _typeidx select 1, if ((_typeidx select 0) in ["basic","specops"]) then {"patrol2mt"} else {"patrol"},GVAR(enemy_side),0,-1.111,1,[_trg_center, _radius]] spawn FUNC(makegroup);
			_wp_array set [_wp_ran, -1];
			_wp_array = _wp_array - [-1];
			waitUntil {sleep 0.112;scriptDone _shandle};
			sleep 0.4;
