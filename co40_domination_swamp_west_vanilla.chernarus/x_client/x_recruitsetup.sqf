...
[_marker_name, getPosASL _D_AI_HUT,"ICON","ColorYellow",[0.5,0.5],"Recruit Barracks",0,"mil_dot"] call FUNC(CreateMarkerLocal);

_D_AI_HUT addAction ["Recruit AI" call FUNC(BlueText), "x_client\x_recruitaiaction.sqf"];
	...
	#endif
	{
		if (!isNil "_x") then {
			if (!isNull _x) then {
				...
				[_marker_name, getPosASL _x,"ICON","ColorYellow",[0.5,0.5],"Recruit Barracks",0,"mil_dot"] call FUNC(CreateMarkerLocal);

				_x addAction ["Recruit AI" call FUNC(BlueText), "x_client\x_recruitaiaction.sqf"];
			};
		};
		#ifdef __A2ONLY__
		__INC(_forEachIndex);
