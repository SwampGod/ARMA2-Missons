		...
		if (!(GV(_vehicle,GVAR(Vehicle_Attached))) && (_pos select 2 > 2.5) && (_pos select 2 < 11)) then {
				...
				if (!_depl) then {
					...
					if ((_px <= _nx + 10 && _px >= _nx - 10) && (_py <= _ny + 10 && _py >= _ny - 10)) then {
						if (!_menu_lift_shown) then {
							_id = _vehicle addAction ["Lift vehicle" call FUNC(BlueText), "x_client\x_heli_action.sqf",-1,100000];
			...
			if (isNull _nearest) then {
				...
				if (GV(_vehicle,GVAR(Vehicle_Attached))) then {
					_release_id = _vehicle addAction ["Release vehicle" call FUNC(RedText), "x_client\x_heli_release.sqf",-1,100000];
