...
if (GVAR(current_seize) != _current_target_name) then {	
	...
	_dtask setSimpleTaskDescription [format ["Seize %1...", _current_target_name], format ["Main Target: Seize %1", _current_target_name], format ["Main Target: Seize %1", _current_target_name]];
...
hint ("Next target is: " + _current_target_name);
