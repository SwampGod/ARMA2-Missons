...
while {true} do {
	...
	switch (_artinum) do {
		case 1: {__pSetVar [QGVAR(ari1), player addAction ["Call Artillery" call FUNC(GreyText), "x_client\x_artillery.sqf",[1,GVAR(AriTarget)],-1,false]]};
		case 2: {__pSetVar [QGVAR(ari1), player addAction ["Call Artillery" call FUNC(GreyText), "x_client\x_artillery.sqf",[2,GVAR(AriTarget2)],-1,false]]};
	};
	while {isNumber(configFile >> "CfgWeapons" >> secondaryWeapon player >> "ACE_is_radio")} do {
		...
		if (vehicle player != player) then {
			...
			if (player != driver _vec && _vec_id == -8882) then {
				switch (_artinum) do {
					case 1: {_vec_id = _vec addAction ["Call Artillery" call FUNC(GreyText), "x_client\x_artillery.sqf",[1,GVAR(AriTarget)],-1,false]};
					case 2: {_vec_id = _vec addAction ["Call Artillery" call FUNC(GreyText), "x_client\x_artillery.sqf",[2,GVAR(AriTarget2)],-1,false]};
				};
			} else {
