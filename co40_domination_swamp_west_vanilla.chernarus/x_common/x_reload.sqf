...
if (!isDedicated) then {
	[_object , format ["Service point stats: fuel available: %1 (in liter), ammo available: %2 (in percent)", round _fuelavailable, round _magspercent]] call FUNC(VehicleChat);
};
...
if (count _magazines > 0) then {
	...
	for "_ee" from 0 to (count _removed - 1) do {
		...
		if (_count > 0) then {
			...
			if (!isDedicated) then {[_object, "Reloading " + _mag_disp_name] call FUNC(VehicleChat)};
			for "_zz" from 1 to _count do {
				...
				if (_magspercent == 0) exitWith {
					if (!isDedicated) then {[_object, "No more magazines available..."] call FUNC(VehicleChat)};
				};
				sleep GVAR(reload_time_factor);
				if (!alive _object) exitWith {};
			};
...
sleep GVAR(reload_time_factor);
if (!alive _object) exitWith {};
if (!isDedicated) then {[_object, "Repairing..."] call FUNC(VehicleChat)};
_object setDamage 0;
sleep GVAR(reload_time_factor);
if (!alive _object) exitWith {};
if (!isDedicated) then {[_object, "Refueling..."] call FUNC(VehicleChat)};
...
_trigger setVariable [QGVAR(fuelavailable), _fuelavailable, true];
if (!alive _object) exitWith {};
if (!isDedicated) then {[_object, _type_name + " is ready..."] call FUNC(VehicleChat)};
