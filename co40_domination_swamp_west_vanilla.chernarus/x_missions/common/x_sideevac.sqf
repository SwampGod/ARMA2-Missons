...
while {!_pilots_at_base && !_is_dead} do {
	...
	if (!alive _pilot1 && !alive _pilot2) then {
		...
		if (!_rescued) then {
			...
			if (alive _pilot2) then {[_nobjs, (position _pilot2) nearObjects ["Man", 20]] call FUNC(arrayPushStack)};
			if (count _nobjs > 0) then {
				{
					if (isPlayer _x) exitWith {
						...
						if (alive _pilot1) then {
							...
							[QGVAR(joing), [[_pilot1], group _x]] call FUNC(NetCallEvent);
						};
						if (alive _pilot2) then {
							_pilot2 setUnitPos "AUTO";
							_pilot2 enableAI "MOVE";
							[QGVAR(joing), [[_pilot2], group _x]] call FUNC(NetCallEvent);
						};
					};
					sleep 0.01;
				} forEach _nobjs;
	...
	if (_pilots_at_base) then {
		if (GVAR(with_ranked)) then {
			if !(__TTVer) then {
				...
				if (_winner == 1) then {
					[QGVAR(sm_p_pos), position EFLAG_BASE] call FUNC(NetCallEvent);
				} else {
					[QGVAR(sm_p_pos), position WFLAG_BASE] call FUNC(NetCallEvent);
				}
			};
		};
		if (_winner != 0) then {
