...
_grps = ["specops", 3, "basic", 0, _poss , 50, true] call FUNC(CreateInf);
_units = [];
{_units = [_units, units _x] call FUNC(arrayPushStack)} forEach _grps;

GVAR(num_species) = 0;
#ifdef __TT__
{_x addEventHandler ["killed", {_this call FUNC(AddSMPoints)}]} forEach _units;
#endif
{_x allowFleeing 0;_x addEventHandler ["killed", {__INC(GVAR(num_species))}]} forEach _units;
sleep 2.123;
