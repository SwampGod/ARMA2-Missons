...
sleep 2.123;
["shilka", 1, "bmp", 1, "tank", 1, _ran_pos,1,110,true] spawn FUNC(CreateArmor);
sleep 2.123;
["specops", 2, "basic", 1, _ran_pos,100,true] spawn FUNC(CreateInf);
...
while {true} do {
	...
	if (!isNull _owner && (_owner distance FLAG_BASE < 20)) exitWith {
		if (__RankedVer) then {[QGVAR(sm_p_pos), position FLAG_BASE] call FUNC(NetCallEvent)};
	...
	if (!isNull _owner && (_owner distance EFLAG_BASE < 20)) exitWith {
		if (__RankedVer) then {[QGVAR(sm_p_pos), position EFLAG_BASE] call FUNC(NetCallEvent)};
		_flag setFlagOwner objNull;
		deleteVehicle _flag;
	...
	if (!isNull _owner && (_owner distance WFLAG_BASE < 20)) exitWith {
		if (__RankedVer) then {[QGVAR(sm_p_pos), position WFLAG_BASE] call FUNC(NetCallEvent)};
		_flag setFlagOwner objNull;
		deleteVehicle _flag;
		GVAR(side_mission_winner) = 2;
		GVAR(side_mission_resolved) = true;
