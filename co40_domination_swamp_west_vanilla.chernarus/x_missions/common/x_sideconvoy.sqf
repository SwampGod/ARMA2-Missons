...
GVAR(extra_mission_vehicle_remover_array) = [GVAR(extra_mission_vehicle_remover_array), _vehicles] call FUNC(arrayPushStack);
...
while {true} do {
	...
	if (GVAR(with_ranked)) then {[QGVAR(sm_p_pos), position _leader] call FUNC(NetCallEvent)};
