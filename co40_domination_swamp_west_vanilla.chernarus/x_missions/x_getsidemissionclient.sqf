...
if (__XJIPGetVar(GVAR(current_mission_index)) != -1) then {
	...
	if (GVAR(x_sm_type) != "convoy") then {
		...
		[_m_name,_posione,"ICON","ColorRed",[1,1],"Side Mission",0,"hd_destroy"] call FUNC(CreateMarkerLocal);
		if (GVAR(with_ranked)) then {
			_posione spawn {
				private "_posione";
				_posione = _this;
		...
		_m_name = format ["XMISSIONM%1", __XJIPGetVar(GVAR(current_mission_index)) + 1];
		[_m_name, _posione,"ICON","ColorRed",[1,1],"Side Mission, Convoy Start",0,"hd_start"] call FUNC(CreateMarkerLocal);
		_m_name = format ["XMISSIONM2%1", __XJIPGetVar(GVAR(current_mission_index)) + 1];
		_posione = _posi_array select 1;
		[_m_name, _posione,"ICON","ColorRed",[1,1],"Side Mission, Convoy End",0,"mil_pickup"] call FUNC(CreateMarkerLocal);
	};
};
