...
[0, QGVARXR(cpr), {if (local _this) then {_this call FUNCXR(cpr)}}] call d_fnc_NetAddEvent;
[1, QGVARXR(addpoint), {
	private ["_killer", "_killed", "_par", "_namep", "_namek", "_killedfriendly"];
...
[1, QGVARXR(subpoint), {(_this select 0) addScore (d_sub_tk_points * -1)}] call d_fnc_NetAddEvent;
#ifndef __TT__
[2, QGVARXR(addpoint), {
	if (isPlayer (_this select 0)) then {
		(format ["%2 is unconscious. Hit by %1", name (_this select 0), name (_this select 1)]) call d_fnc_GlobalChat;
	} else {
		(format ["%1 is unconscious.", name (_this select 1)]) call d_fnc_GlobalChat;
	};
}] call d_fnc_NetAddEvent;
#endif
...
if (d_sub_kill_points != 0) then {
	...
	[1, QGVARXR(add_sub_kill), {_this addScore d_sub_kill_points}] call d_fnc_NetAddEvent;
};

[2, QGVARXR(unconmsg), {
	(format ["%1 is unconscious.", name _this]) call d_fnc_GlobalChat;
}] call d_fnc_NetAddEvent;
[2, QGVARXR(subpoint), {
	if (isPlayer (_this select 0)) then {
		(format ["%2 is unconscious. Hit by %1", name (_this select 0), name (_this select 1)]) call d_fnc_GlobalChat;
...
[2, QGVARXR(msg), {
	if ((_this select 2) == side (group player)) then {
		GVARXR(msg_logic) globalChat format ["%1 was revived by %2", _this select 1, _this select 0];
	};
	if ((_this select 1) == GVARXR(name_player)) then {
		_this spawn {
			sleep 2;
			GVARXR(msg_logic) globalChat ("You were revived by: " + (_this select 0));
		};
	};
}] call d_fnc_NetAddEvent;
...
#ifdef __TT__
[2, QGVARXR(addActions), {if (player != (_this select 0) && (_this select 1) == side (group player)) then {(_this select 0) call FUNCXR(addActions)}}] call d_fnc_NetAddEvent; // TODO, what if more than one side is friendly
#else
[2, QGVARXR(addActions), {if (player != _this) then {_this call FUNCXR(addActions)}}] call d_fnc_NetAddEvent;
#endif
[2, QGVARXR(removeActions), {if (player != _this) then {_this call FUNCXR(removeActions)}}] call d_fnc_NetAddEvent;
#ifndef __A2XRONLY__
[0, QGVARXR(hpl), {_this hideobject true}] call d_fnc_NetAddEvent;
#endif
[0, QGVARXR(grpl), {if (local (leader (_this select 0))) then {(_this select 0) selectLeader (_this select 1)}}] call d_fnc_NetAddEvent;
...
[2, QGVARXR(doHandleHeal), {if (local _this) then {call FUNCXR(dohandleheal)}}] call d_fnc_NetAddEvent;

if (GVARXR(withSounds)) then {
	[0, QGVARXR(say3d), {(_this select 0) say3D (_this select 1)}] call d_fnc_NetAddEvent;
};
...
if (GVARXR(with_marker)) then {
	...
	FUNCXR(addmarker) = {
		...
		if (side (group _this) == GVARXR(side_pl)) then {
			[_mname, _pos, "ICON", "ColorBlue", [0.4,0.4], (_name + " is down"), 0, "Flag1"] call FUNCXR(CreateMarkerLocal);
		};
	};
	FUNCXR(movemarker) = {
		if (!alive _this) exitWith {};
			...
			if (GVARXR(pl_can_revive)) then {
				if (GVARXR(help_bonus) > 0 && GVARXR(max_lives) != -1) then {
					if (!d_with_ranked) then {
						hintSilent format ["You get %1 extra life(s) for reviving another player", GVARXR(help_bonus)];
					} else {
						hintSilent format ["You get %1 extra life(s) and %2 points for reviving another player", GVARXR(help_bonus), d_ranked_a select 21];
						["d_pas", [player, d_ranked_a select 21]] call d_fnc_NetCallEvent;
					};
				...
				if (GVARXR(revivemsg)) then {
					[QGVARXR(msg), [GVARXR(name_player), name __pGetVar(GVARXR(cursorTarget)), side (group player)]] call d_fnc_NetCallEvent;
				};
			} else {
				[QGVARXR(cpr), __pGetVar(GVARXR(cursorTarget))] call d_fnc_NetCallEvent;
			};
		};
	};
...
FUNCXR(respawneh) = {
	...
	if (count _d_pos > 0) then {
		...
		if (!__pGetVar(GVARXR(isdead))) then {
			player playMoveNow (player call FUNCXR(getUnitDeathAnim));
...
FUNCXR(WeapRespawn2) = {
	...
	if (__pGetVar(GVARXR(ubp)) != "") then {
		...
		if (count (__pGetVar(GVARXR(_oabackpackmags)) select 0) > 0) then {
			_mags = __pGetVar(GVARXR(_oabackpackmags)) select 0;
			_mcount = __pGetVar(GVARXR(_oabackpackmags)) select 1;
			{_bp addMagazineCargo [_x, _mcount select _forEachIndex]} forEach _mags;
		};
		if (count (__pGetVar(GVARXR(_oabackpackweaps)) select 0) > 0) then {
			_weaps = __pGetVar(GVARXR(_oabackpackweaps)) select 0;
			_mcount = __pGetVar(GVARXR(_oabackpackweaps)) select 1;
...
FUNCXR(MapClickRespawn) = {
	...
	titleText ["You have selected a respawn position, stand by!","BLACK OUT", 1];
	...
	_nos = nearestObjects [_respawn_pos, ["All"], 15];
	...
	_mhqobj = objNull;
	{
		...
		#else
		_hsv = _x getVariable "d_vec_type";
		if (isNil "_hsv") then {_hsv = ""};
		if (_hsv == "MHQ") exitWith {
			_mhqobj = _x;
		};
		#endif
	} forEach _nos;
	...
	titleText ["You have respawned","BLACK IN", 6];
	if (GVARXR(max_lives) != -1) then {
		sleep 3;
		if (__pGetVar(GVARXR(lives)) > -1) then {
			hintSilent (str __pGetVar(GVARXR(lives)) + " lives left");
		};
	};
...
FUNCXR(joingr) = {
	...
	if (__pGetVar(GVARXR(isleader))) then {
		[QGVARXR(grpl), [GVARXR(pl_group), player]] call d_fnc_NetCallEvent;
	};
...
FUNCXR(MapClickRespawn2) = {
	...
	if (_dist < 30 && toUpper _mcol == "COLORGREEN") exitWith {
		...
		titleText ["You have respawned","BLACK IN", 5];
		if (__pGetVar(GVARXR(isleader))) then {
			[QGVARXR(grpl), [group player, player]] call d_fnc_NetCallEvent;
		};
	};
	_mes = if (_dist < 30) then {_nearest select 3} else {""};
	if (_mes != "") then {
...
FUNCXR(waterfix) = {
	...
	titleCut ["Trying to find a position at land... Stand by...", "PLAIN"];
...
FUNCXR(spectating) = {
	...
	private ["_withresp", "_disp", "_pllist", "_plnamelist", "_u", "_uc", "_add", "_idx", "_forEachIndex", "_campos", "_updatelb", "_timer", "_h", "_fangle", "_fangleY", "_sdistance", "_setidx", "_bb", "_foo", "_hstr", "_vpmtw", "_cxpos", "_cypos", "_czpos", "_gjp", "_lsdist", "_l", "_d", "_z", "_co","_l", "_player_has_no_lifes", "_plposm"];
	...
	if (_withresp) then {
		titleText ["You are seriously wounded!\nWait for help or select a respawn position by map click (when available)","BLACK OUT", 1];
		sleep 5;
		titleText ["","BLACK IN", 1];
	};
	...
	if (!_withresp) then {
		...
		if (GVARXR(respawn_available)) then {
			__spectdlg1006e ctrlSetText "Yes";
	...
	if (!_player_has_no_lifes) then {
		...
		_nposvis = if (abs _tdist < 1) then {
			_tmppos
		} else {
			[_tmppos select 0, _tmppos select 1, _tatlp]
		};
		#else
		_nposvis = _vkegs modelToWorld [0, 0, 0];
		#endif
	...
	while {!GVARXR(stopspect)} do {
		...
		if (ctrlShown (__uiGetVar(GVARXR(SpectDlg)) displayCtrl 1000) && _updatelb) then {
		...
		// user pressed ESC
		if (!ctrlShown (__uiGetVar(GVARXR(SpectDlg)) displayCtrl 1002) && !GVARXR(stopspect)) then {
			...
			if (!_player_has_no_lifes) then {
				...
				#else
				_nposvis = (vehicle _visobj) modelToWorld [0, 0, 0];
				#endif
				_campos = [(_nposvis select 0) - 1 + random 2, (_nposvis select 1) - 1 + random 2, 2];
				GVARXR(spectcam) = "camera" camCreate _campos;
				GVARXR(spectcamtarget) = _visobj;
		...
		#else
		_vpmtw = _vkegs modelToWorld [0, 0, 0];
		#endif
...
FUNCXR(addActions) = {
	...
	if (_hasaction == -9999) then {
		_reviveorcprs = if (GVARXR(pl_can_revive)) then {"Revive"} else {"CPR"};
...
FUNCXR(mobRespAvailable) = {
	...
	switch (true) do {
		case (X_JIPH getVariable _mrinair): {
			_mrtext = format ["Mobile respawn %1 gets transported by airlift...", _mrnumstr];
			_mr_available = false;
		};
		case (speed _mrvec > 4): {
			_mrtext = format ["Mobile respawn %1 currently driving...", _mrnumstr];
			_mr_available = false;
		};
		case (surfaceIsWater [(position _mrvec) select 0,(position _mrvec) select 1]): {
			_mrtext = format ["Mobile respawn %1 is currently in water...", _mrnumstr];
			_mr_available = false;
		};
		case (!(if (!isNil {__GV(_mrvec,ace_canmove)}) then {_mrvec call ace_v_alive} else {alive _mrvec})): {
			_mrtext = format ["Mobile respawn %1 destroyed...", _mrnumstr];
			_mr_available = false;
		};
		default {
			...
			if (!_depl) then {
				_mrtext = format ["Mobile respawn %1 not deployed...", _mrnumstr];
				_mr_available = false;
			} else {
				_enear = __GV(_mrvec,d_enemy_near);
				if (!isNil "_enear") then {
					if (_enear) then {
						_mrtext = format ["Enemies near mobile respawn %1...", _mrnumstr];
						_mr_available = false;
					};
				};
			};
...
FUNCXR(park_player) = {
	...
	titleText ["No lives left...","BLACK OUT", 1];
	if (_dosleep) then {
		sleep 5;
	...
	titleText ["No lives left...","BLACK IN", 1];
};
...
FUNCXR(uncon) = {
	...
	enableRadio false;
	#ifdef __TT__
	[QGVARXR(addActions), [player, side (group player)]] call d_fnc_NetCallEvent;
	#else
	[QGVARXR(addActions), player] call d_fnc_NetCallEvent;
	#endif
	...
	while {alive player && __pGetVar(GVARXR(pluncon))} do {
		if (GVARXR(withSounds)) then {
			if (time >= _nextcrytime) then {
				...
				[QGVARXR(say3d), [player, "Hit" + str(_nus3)]] call d_fnc_NetCallEvent;
				_nextcrytime = time + 10 + (random 15);
			};
		};
		...
		if (GVARXR(near_player_dist_respawn) && !GVARXR(respawn_available) && _dcounter > 10) then {
			...
			if (isNull _nearunit) then {
				...
				if (!GVARXR(followcam)) then {
					...
					__spectdlg1006 ctrlSetText "Yes";
		...
		if (_tt <= 0) exitWith {
			...
			[QGVARXR(removeActions), player] call d_fnc_NetCallEvent;
			_remactions = true;
			__pSetVar [QGVARXR(pluncon), false, true];
			...
			if (GVARXR(max_lives) != -1) then {
				hintSilent format ["%1 life(s) left", __pGetVar(GVARXR(lives))];
			};
			// deactivated for now
			//if (!GVARXR(no_respawn_dialog)) then {
			//	createDialog QGVARXR(RespDlg);
...
FUNCXR(ClientHD) = {
			...
			if (isPlayer _injurer) then {
				if (_injurer != _unit && side (group _injurer) == side (group _unit) && getText (configFile >> "CfgAmmo" >> _ammo >> "simulation") in __shots) then {
					if (_part == "") then {
						...
						["d_unit_tkr", [_unit,_injurer]] call d_fnc_NetCallEvent;
					};
					_ddexit = true;
				};
			};
	...
	if (_part == "") then {
		...
		if (_vec == _unit) then {
			_ndoad = (_difd * 0.8) min 0.9;
			_oad = (__GV(_unit,GVARXR(overall)) + _ndoad) min 0.9;
			...
			if (!alive _vec) then {
				...
				if (_difd > 0.15) then {
					if (time >= __GV(_unit,GVARXR(cutofftime))) then {
						...
						_pd = (__GV(_unit,GVARXR(overall)) + _difd) min 0.9;
						_unit setVariable [QGVARXR(overall), _pd];
						_unit setDamage _pd;
						if (_pd >= 0.9) exitWith {
							_vecdead = true;
						};
						_pd = (__GV(_unit,GVARXR(head_hit)) + _difd) min 0.9;
						_unit setVariable [QGVARXR(head_hit), _pd];
						_pd = (__GV(_unit,GVARXR(body)) + _difd) min 0.9;
		...
		switch (_part) do {
			case "head_hit": {
				...
				_nd = (_dam * (0.6 + (1 - _armor))) min 0.9;
				_newpd = (__GV(_unit,GVARXR(head_hit)) + _nd) min 0.9;
			...
			case "body": {
				...
				_nd = if (_passt < 1 && _armor == 1) then {
					...
					(_dam * (0.6 + (1 - _armor))) min 0.9
				};
				_newpd = (__GV(_unit,GVARXR(body)) + _nd) min 0.9;
	...
	if (_isbh != 0 || _vecdead) then {
		if ((_isbh == 1 && _ret >= 0.9) || (_isbh == 2 && _ret >= 15) || _vecdead) then {
			...
			if (isPlayer _injurer && _injurer != player) then {
				if (side (group _injurer) != _sideplg) then {
					[QGVARXR(addpoint), [_injurer, _unit]] call d_fnc_NetCallEvent;
				} else {
					[QGVARXR(subpoint), [_injurer, _unit]] call d_fnc_NetCallEvent;
				};
			} else {
				[QGVARXR(unconmsg), _unit] call d_fnc_NetCallEvent;
			};
			if (d_sub_kill_points != 0) then {
				[QGVARXR(add_sub_kill), player] call d_fnc_NetCallEvent;
			};
			_isInVec = false;
...
FUNCXR(makeUncon) = {
	player setVariable [QGVARXR(overall), 0.9];
...
FUNCXR(drag) = {
	...
	_anim = (player call FUNCXR(getUnitAnim)) select 0;
