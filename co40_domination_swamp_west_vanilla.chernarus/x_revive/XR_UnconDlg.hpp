...
class GVARXR(UnconDlg) {
	idd = -1;
	movingEnable = 1;
	onLoad = "uiNamespace setVariable ['XR_UnconDlg', _this select 0];xr_lb_u_updater = true;0 spawn xr_fnc_pllbupdater";
	onUnLoad = "xr_lb_u_updater = false";
	objects[] = {};
	...
	class controls {
		class FirstText: GVARXR(SXRscText) {
			...
			sizeEx = 0.05;
			colorText[] = {1, 1, 1, 1};
			text = "You are seriously wounded!";
		};
		class SecondText: FirstText {
			...
			w = 0.7;
			sizeEx = 0.023;
			text = "Wait for help or select a respawn position by map click (when available)";
		};
		...
		class RespawnTextCap: GVARXR(SXRscText) {
			...
			sizeEx = 0.023;
			colorText[] = {1, 1, 1, 1};
			text = "Map click respawn available:";
		};
