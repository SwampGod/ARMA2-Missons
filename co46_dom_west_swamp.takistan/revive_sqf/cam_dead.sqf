...
_allUnits =  allUnits;
{if (side _x != sideLogic && (_x distance _boot_hill) > 100 && (_x distance _respawn_mrkr) > 100 && alive _x) then {_alive_friends set [count _alive_friends, _x]}} forEach _allUnits;
...
while {!_destroy} do {
	...
	if (_c == 0) then {
		_allUnits =  allUnits;
		_new_friends = [];
		{if (side _x != sideLogic && (_x distance _boot_hill) > 100 && (_x distance _respawn_mrkr) > 100 && alive _x) then {_new_friends set [count _new_friends, _x]}} forEach _allUnits;
