...
{if (_x getVariable "NORRN_AIunconscious") then {_unc_array set [count _unc_array, _x]}}forEach _close_array;

_revive_units = [];
_protect_units = [];
{
	_ur = missionNamespace getVariable _x;
	if (!(_ur getVariable "NORRN_unconscious") && !(_ur getVariable "NORRN_AI_help")) then {
		if (_ur isKindOf _can_revive && _medpacks  == 0 || _ur isKindOf _can_revive_2 && _medpacks  == 0 || _ur isKindOf _can_revive && _medpacks == 1 && (_ur getVariable "NORRN_medpacks") > 0 || 
		_ur isKindOf _can_revive_2 && _medpacks  == 1 && (_ur getVariable "NORRN_medpacks") > 0 || _ur isKindOf _medic_1 && _medpacks == 1 && (_ur getVariable "NORRN_medpacks") > 0 || 
		_ur isKindOf _medic_2 && _medpacks  == 1 && (_ur getVariable "NORRN_medpacks") > 0) then {
			_revive_units set [count _revive_units, _ur]
		};
		if (_ur isKindOf _can_revive || _ur isKindOf _can_revive_2 || _ur isKindOf _medic_1 && _medpacks == 1 || _ur isKindOf _medic_2 && _medpacks == 1) then {
			_protect_units set [count _protect_units, _ur];
		};
	};
} forEach _units;

{if !(_x in _protect_units) then {_revive_array set [count _revive_array, _x]}}forEach _unc_array;
